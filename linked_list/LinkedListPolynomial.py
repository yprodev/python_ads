...
class Polynomial:
	...
	# Polynomial addtion: newPoly = self + rhsPoly
	...
	def multiply(self, rhsPoly):
		...
		# Iterate through the remaining terms of the poly computing the product
		# of the rhsPoly by each term
		node = node.next
	...
	# Helper method for creating a new polynomial from multiplying
	# an existing polynomial by another term
	def _termMultiply(self, termNode):
		newPoly = Polynomial()

		# Iterate through the terms and compute the product of each term and
		# the term in termNode
		curr = curr.next # ??? Where is curr???
	...
	# Polynomial multiplication: newPoly = self * rshPoly
