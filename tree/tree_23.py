class Tree23Map:
	def _23Search(subtree, target):
		# If we encounter a null pinter, the target is not in the tree
		if subtree is None:
			return None
	...
	def _23Insert(self, key, newitem):
		...
		if self._root is None:
			self._root = _23TreeNode(kew, newitem)
	...
	def _23AddToNode(self, subtree, key, data, pRef):
		...
		# Otherwise, add the new key in its proper order
			...
			if key < subtree.key1:
				...
				subtree.key1 = key
				subtree.data1 = data
				if pRef is not None: # if interior node, se the links
	...
	def _23SplitNode(self, node, key, data, pRef):
		...
		newnode = _23SplitNode(None, None)
...
class _23TreeNode(object):
	...
	def hasKey(self, target):
		if (target == self.key1) or (self.key is not None and target == self.key2):
			return True
		else:
			return False
